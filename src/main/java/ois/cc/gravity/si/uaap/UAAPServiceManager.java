package ois.cc.gravity.si.uaap;
...
import ois.cc.gravity.services.RequestContext;
...
public class UAAPServiceManager
{
    ...
    public void AppLogin() throws Exception, GravityRuntimeCheckFailedException
    {
    ...
    public Event EvaluateUaap(Request request, UAClient uac) throws Exception, CODEException, GravityException
    {
        ...
        if (uac.getUserType().equals(UserRole.System))
        {
            return null;
        }

        if (!uac.getUserType().equals(UserRole.Admin))
        {
            return null;
        }
        ...
        if (request.getReqType().equals(GReqType.Config)
                && !enName.equals(EN.Profile) && !enName.equals(EN.UserGroup)
                && !enName.equals(EN.User) && !enName.equals(EN.UserProfile))
        {

            String entityName = enName.name();
            String action = request.getReqCode().toString();

            JPAQuery qry = new JPAQuery("Select a.Id From AdminGroup a Where a.Admins.Id =: aid");
            qry.setParam("aid", uac.getUserId());
            ArrayList<Long> adGpIds = uac.getTenantContext().getDB().SelectList(qry);
    ...
    public code.uaap.service.event.Event AddPolicy(TenantContext tctx, Policy policy) throws Exception, GravityRuntimeCheckFailedException
    {

        code.uaap.service.event.Event ev = getUAAPClient(tctx.getTenant().getCode()).AddPolicy(policy);
    ...
    public code.uaap.service.event.Event EditPolicy(TenantContext tctx, Policy policy) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(tctx.getTenant().getCode()).EditPolicy(policy);
    ...
    public code.uaap.service.event.Event DeletePolicy(TenantContext cctx, String policy) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).DeletePolicy(policy);
    ...
    public code.uaap.service.event.Event MapPolicyUser(TenantContext cctx, String policycode, long userid) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).MapPolicyUser(policycode, userid);
    ...
    public code.uaap.service.event.Event UnmapPolicyUser(TenantContext cctx, String policycode, long userid) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).UnmapPolicyUser(policycode, userid);
    ...
    public code.uaap.service.event.Event MapPolicyUsergroup(TenantContext cctx, String policycode, long usergroupid) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).MapPolicyUsergroup(policycode, usergroupid);
    ...
    public code.uaap.service.event.Event UnmapPolicyUsergroup(TenantContext cctx, String policycode, long usergroupid) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).UnmapPolicyUsergroup(policycode, usergroupid);
    ...
    public code.uaap.service.event.Event FetchPolicy(TenantContext cctx, HashMap<String, ArrayList<String>> filter) throws Exception, GravityRuntimeCheckFailedException
    {
        code.uaap.service.event.Event ev = getUAAPClient(cctx.getTenant().getCode()).FetchPolicy(filter);
    ...
    public ArrayList<Policy> FetchPoliciesByCode(TenantContext cctx, Profile profile) throws Exception, GravityRuntimeCheckFailedException
    {
