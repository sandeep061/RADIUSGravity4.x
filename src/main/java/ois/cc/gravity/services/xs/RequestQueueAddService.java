...
public class RequestQueueAddService extends RequestEntityAddService
{

    public RequestQueueAddService(UAClient uac)
    {
        super(uac);
    }
...
//        }
        ...
        _tctx.getDB().FindAssert(EN.XServer.getEntityClass(), xid);
        //check address is unique or not
        if (req.getAttributes().containsKey("Address"))
        {
             String address=req.getAttributeValueOf(String.class,"Address");
           Queue existqueue=_tctx.getDB().Find(new QueueQuery().filterByAddress(address).filterByXServer(xid));
            if (existqueue!=null){
                GravityEntityExistsException reex = new GravityEntityExistsException(existqueue.getClass().getSimpleName(), "Queue", OPRelational.Eq, existqueue.getId());
                throw reex;
            }
        }
